...
const addAppointment = async (req, res) => {
    try {
      ...
      if(req.body.dateAndTime) {
        validateDateAndTime = Date.parse(req.body.dateAndTime);
    ...
    try {
      ...
      if (!appointment) {
        ...
        if (req.user.userId === appointment.createdBy.toString()) {
          ...
          const updatedAppointment = await Appointment.findOneAndUpdate({ _id: appointmentId }, req.body, {
            new: true,
...
const deleteAppointment = async (req, res) => {
    ...
    // CHECK THAT DATE STRING IS VALID
    if(req.body.dateAndTime) {
        validateDateAndTime = new Date(req.body.dateAndTime);
